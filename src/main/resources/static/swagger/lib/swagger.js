...
SwaggerAuthorizations.prototype.apply = function(obj, authorizations) {
  ...
  /*** code by wangxinyuan for add authorization to interact with ticket service ***/

  /******** start ****************/

  var ak = this.authz['ak'];
  var sk = this.authz['sk'];
  var params = this.authz['params'];
  var requestVersion = "1";
  var expirationPeriodInSecond = 3600;


  if(params!=null) {
    obj.url = obj.url + "?" + params.value;
  }
  ...
  if (ak != null && sk != null) {
    ...
    var authorization = "bce-auth-v" + requestVersion + "/" +
        ak.value + "/" + (utcDate).format("yyyy-MM-ddThh:mm:ssZ") + "/" +
        expirationPeriodInSecond;

    var signingKey = HMAC_SHA256_MAC(sk.value, authorization);
    log("signingKey: " + signingKey);
     ...
     if(queryIndex != -1) {
        path = url.substring(queryIndex , -1);
        canonicalQueryString = url.substring(queryIndex + 1);

        if (canonicalQueryString.length > 0) {

            var temp = "";
            var params = canonicalQueryString.split('&');
     ...
     var xBceDate = utcDate.format("yyyy-MM-ddThh:mm:ssZ");
     obj.headers['x-bce-date'] = xBceDate;

     var canonicalHeader = "host:" + encodeURIComponent(location.host) + "\n" +
        "x-bce-date:" + encodeURIComponent(xBceDate);

     var canonicalRequest = obj.method.toUpperCase()+ "\n" +
                canonicalURI + "\n" +
                canonicalQueryString + "\n" +
                canonicalHeader;

     var signature = HMAC_SHA256_MAC(signingKey,canonicalRequest);

     authorization += "/" +
                    signedHeaders + "/" +
                    signature;

     var val = new ApiKeyAuthorization();
     val.name = "authorization";
     val.value = authorization;
     val.type = "header";
     result = val.apply(obj, authorizations);
     if (result === false)
       status = false;
     if (result === true)
       status = true;
